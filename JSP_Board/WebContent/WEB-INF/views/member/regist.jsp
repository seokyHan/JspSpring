				...
				<div class="register-card-body">
					<form role="form" class="form-horizontal" action="regist.do" method="post">						
						...
						<div class="input-group mb-3">
								...
								<div class="mailbox-attachment-info">
									<div class="input-group input-group-sm">
										...
										<span class="input-group-append-sm">											
											<button type="button" class="btn btn-info btn-sm btn-append" onclick="pictureUpload();">업로드</button>											
										</span>
									</div>
								</div>
							</div>
						  ...
						  <div class="form-group row">
							...
							<div class="col-sm-9 input-group input-group-sm">
								<input name="id" onkeyup="this.value=this.value.replace(/[\ㄱ-ㅎㅏ-ㅣ가-힣]/g,'');" autocomplete="off"
										type="text" class="form-control" id="id" placeholder="13글자 영문자,숫자 조합" />
								<span class="input-group-append-sm">	
									<button type="button" onclick="idCheck();"  class="btn btn-info btn-sm btn-append">중복확인</button>
								</span>								
							</div>								
						</div>
						<div class="form-group row">
						...
						<div class="form-group row">
							...
							<div class="col-sm-9 input-group-sm">								
								<input class="form-control" name="name" type="text" class="form-control" id="name"
										placeholder="이름을 입력하세요" autocomplete="off" onkeyup="this.value=this.value.trim();" />
							</div>
						...
						<div class="form-group row">
							...
							<div class="col-sm-9 input-group-sm">
								<input name="email" type="email" class="form-control" id="email"
										autocomplete="off" placeholder="example@naver.com">
							</div>
						</div>
						<div class="form-group row">
						...
						<div class="card-footer">
							<div class="row">								
								<div class="col-sm-6">
									<button type="button" id="registBtn" onclick="registMember();" class="btn btn-info">등&nbsp;&nbsp;록</button>
							 	</div>
...
<form role="imageForm" action="upload/picture" method="post" 
	...
	<input id="inputFile" name="pictureFile" type="file" class="form-control"
		   onchange="onPicture();"	style="display:none;">
...
let formData = "";
	...
	const onPicture = function(){
		//alert("file change!");
		
	   const form = $('form[role="imageForm"]');
	   const picture = form.find('[name=pictureFile]')[0];
		   
	   formData = new FormData($('form[role="imageForm"]')[0]);
	   
	   //이미지 확장자 확인
	   const fileFormat = picture.value.substr(picture.value.lastIndexOf(".")+1).toUpperCase();
		if(!(fileFormat=="JPG" || fileFormat=="JPEG" || fileFormat=="GIF")){
	   		alert("이미지는 jpg/jpeg/gif 형식만 가능합니다.");
	   		picture.value="";      
	   		return;
		} 
		...
		$.ajax({
			...
			success:function(result){
				if(result.toUpperCase() === "DUPLICATED"){
		            ...
		            $('input[name="id"]').val(id.val().trim());
				}
			},
			error:function(error){
				alert('시스템장애로 인하여 서비스 불가합니다');
			}
		});
	}
	...
	const registMember = function(){
		...
		const uploadCheck = $('input[name="checkUpload"]').val();   
		if(uploadCheck=="0"){
		     alert("사진업로드는 필수 입니다");      
		     return;
		}
